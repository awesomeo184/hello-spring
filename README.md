...
컨트롤러, 서비스, 리포지토리에 각각 @Controller, @Service, @Repository 애노테이션을 달아주면 프로젝트를 시작할 때, 스프링 빈에 등록되어, 컨테이너에 담겨서 관리됨.
...
멤버 컨트롤러는 멤버 서비스를 이용한다.

멤버 서비스는 멤버 리포지토리를 이용한다

이들을 연결해주기 위해서는 생성자 위에 @Autowired 애노테이션을 달아주어야 함.



## 13강 자바 코드로 직접 빈 등록

Configuration을 만들고 @Bean 애노테이션을 붙여서 직접 등록한다.

구현체를 나중에 변경할 때 이 방식을 이용하면 다른 코드를 건드리지 않고 Configuration 파일만 변경하여 바꿀 수 있음.
